...
class Demo(object):
    ...
    def parse(self):
        ...
        while True:
            ...
            cmd_meta = self.file.read(12)

            if len(cmd_meta) == 0:
                break

            cmd,      cmd_pos = parser.decode_varint(cmd_meta,       0)
            cmd_tick, cmd_pos = parser.decode_varint(cmd_meta, cmd_pos)
            cmd_size, cmd_pos = parser.decode_varint(cmd_meta, cmd_pos)

            self.file.seek(cmd_tell + cmd_pos, 0)

            compressed = (demo_pb2.DEM_IsCompressed & cmd == demo_pb2.DEM_IsCompressed)
